...
with st.sidebar:
    ...
    if df_master is not None and 'Agency' in df_master.columns:
        ...
        selected_agency = st.selectbox("Select Core Reporting Agency:", ["All Agencies"] + list(agency_opts), key="sb_agency_v10")
        if selected_agency != "All Agencies" and df is not None:
            df = df[df['Agency'] == selected_agency]

    # 2. Operational Region Filter (STRICT MATCH FOR LOWERCASE 'region')
    if df_master is not None and 'region' in df_master.columns:
        region_opts = sorted(df_master['region'].dropna().unique())
        selected_region = st.selectbox("Select Operational Region:", ["All Region"] + list(region_opts), key="sb_region_v10")
    ...
    if df_master is not None and 'Retailer' in df_master.columns:
        ...
        selected_retailer = st.selectbox("Select Active Retailer:", ["All Retailers"] + list(retailer_opts), key="sb_retailer_v10")
    ...
    if df_master is not None and 'Command Tier' in df_master.columns:
        ...
        selected_command_tier = st.selectbox("Select Command Tier:", ["All Command Tiers"] + list(command_tier_opts), key="sb_command_tier_v10")
